<template>
            ...
            <div class="tab-content">
                <div class="tab-pane fade active show" id="tab-eg-55">
                    <div class="pt-2 card-body">
                        <div class="row">
                          <form action="" class="form-all" :class="{ active: isActive }" @submit.prevent="editForm">
  ...
  export default defineComponent({
    ...
    axios.get(`http://localhost:9000/api/v1/cms/users/${id}`)
      .then(response => {
        // Isi data user ke dalam objek user
  ...
  methods: {
    ...
    async editForm(newUser: Omit<User, '_id'>, imageId: string) {
  try {
    const idku = this.$route.params.id;
    const response = await axios.put(
      `http://localhost:9000/api/v1/cms/users/${idku}`,
      {
        ...newUser,
        image: imageId,
      }
    );
    console.log('User edited:', response.data.data);
